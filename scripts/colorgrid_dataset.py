...
RAW_DIR = os.path.realpath(os.path.join(FILE_DIR, '/mnt/fs5/hokysung/datasets/colorgrid_data/'))
...
class WeakSup_ColorGridDataset(ColorGridDataset):
...
class ColorGrid_ReferenceGame(data.Dataset):
...
class WeakSup_ColorGridReference(ColorGrid_ReferenceGame):
    def __init__(self, vocab=None, split='Train', supervision_level=1.0, context_condition='far'):
        super(WeakSup_ColorGridReference, self).__init__(vocab=vocab, split=split, context_condition=context_condition)
...
ColorGridDataset(vocab=None, split='Train', context_condition='far')
