...
class ColorDataset(data.Dataset):
    def __init__(self, vocab=None, split='Train'):
        ...
        if split == 'Train':
            self.texts = self.textsList[:int(length * TRAINING_PERCENTAGE)]
            self.rounds = self.roundsList[:int(length * TRAINING_PERCENTAGE)]
            self.images = self.imagesList[:int(length * TRAINING_PERCENTAGE)]
        elif split == 'Validation':
            self.texts = self.textsList[int(length * TRAINING_PERCENTAGE):-int(length * TESTING_PERCENTAGE)]
    ...
    def concatenate_by_round(self, texts, images, rounds):
        ...
        for i in range(1, len(rounds)):
            if rounds[i] == rounds[i-1]:
                ...
                rawRGB = np.array(self.hsl2rgb(images[i-1]))
