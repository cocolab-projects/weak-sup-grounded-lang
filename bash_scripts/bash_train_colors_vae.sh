...
do
	...
	do
		screen -S train_colors_alpha_${ALPHA}_beta_${BETA}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=7 python train_colors_vae.py ${OUT_DIR}_alpha_beta_re ${SUP_LEV} --dropout ${DROPOUT} --seed 50 --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda; exec bash";
...
do
	...
	do
		screen -S train_colors_alpha_${ALPHA}_beta_${BETA}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=8 python train_colors_vae.py ${OUT_DIR}_alpha_beta_re ${SUP_LEV} --dropout ${DROPOUT} --seed 50 --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda; exec bash";
		# screen -S train_colors_${SUP_LEV}_vae_hard -dm bash -c "CUDA_VISIBLE_DEVICES=1 python train_colors_vae.py ${OUT_DIR}_hard ${SUP_LEV} --dropout ${DROPOUT} --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda --hard; exec bash";
	done
done
...
do
	for BETA in 1 2 4 5 8
	do
		screen -S train_colors_alpha_${ALPHA}_beta_${BETA}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=9 python train_colors_vae.py ${OUT_DIR}_alpha_beta_re ${SUP_LEV} --dropout ${DROPOUT} --seed 50 --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda; exec bash";
		# screen -S train_colors_${SUP_LEV}_vae_hard -dm bash -c "CUDA_VISIBLE_DEVICES=1 python train_colors_vae.py ${OUT_DIR}_hard ${SUP_LEV} --dropout ${DROPOUT} --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda --hard; exec bash";
	done
done
