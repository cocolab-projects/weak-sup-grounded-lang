...
do
	for BETA in 1 2 4 5 8 10 20 30 40 50
	do
		screen -S train_colors_alpha_${ALPHA}_beta_${BETA}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=2 python train_colors_vae.py ${OUT_DIR}_alpha_beta ${SUP_LEV} --dropout ${DROPOUT} --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda; exec bash";
		# screen -S train_colors_${SUP_LEV}_vae_hard -dm bash -c "CUDA_VISIBLE_DEVICES=1 python train_colors_vae.py ${OUT_DIR}_hard ${SUP_LEV} --dropout ${DROPOUT} --alpha ${ALPHA} --beta ${BETA} --num_iter 3 --cuda --hard; exec bash";
	done
done
...
do
		screen -S train_colors_${SUP_LEV}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=3 python train_colors_vae.py ${OUT_DIR}_weaksup ${SUP_LEV} --dropout ${DROPOUT} --alpha 1 --beta 10 --num_iter 3 --cuda; exec bash";
		screen -S train_colors_${SUP_LEV}_vae_hard -dm bash -c "CUDA_VISIBLE_DEVICES=4 python train_colors_vae.py ${OUT_DIR}_weaksup_hard ${SUP_LEV} --dropout ${DROPOUT} --alpha 1 --beta 10 --num_iter 3 --cuda --hard; exec bash";
done
