...
do
    screen -S test_chairs_${SUP_LEV}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=8 python test_chairs_vae.py ${LOAD_DIR}_default ${RES_OUT_DIR}_default --sup_lvl ${SUP_LEV} --alpha 1 --beta 2 --num_iter 1 --cuda; exec bash";
done

for SUP_LEV in 0.01 0.02 0.05 0.1 0.2 0.5 1.0
do
    screen -S test_chairs_${SUP_LEV}_vae -dm bash -c "CUDA_VISIBLE_DEVICES=9 python test_chairs_vae.py ${LOAD_DIR}_default ${RES_OUT_DIR}_default --sup_lvl ${SUP_LEV} --alpha 1 --beta 2 --num_iter 1 --cuda; exec bash";
done
